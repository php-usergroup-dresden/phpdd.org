...
final class StripeSuccessRequestHandler extends AbstractRequestHandler implements HandlesPostRequest
{
	...
	public function handle( ProvidesWriteRequestData $request )
	{
		...
		$paymentProvider = new PaymentProvider( PaymentProviders::STRIPE );
		$payerId         = new PayerId( (string)$input->get( 'stripeEmail' ) );
		$paymentId       = new PaymentId( (string)$input->get( 'stripeToken' ) );
		...
		$metaData = array_merge(
			$ticketOrderPayment->getMetaData(),
			[
				'chargeId' => $execResult->getPaymentId()->toString(),
			]
		);

		$this->markPaymentAsExecuted( $ticketOrderRepository, $paymentId, $metaData );
	...
	private function markPaymentAsExecuted(
	...
	) : void
	{
		try
		{
			$ticketOrderRepository->markPaymentAsExecuted( $paymentId, new DateTimeImmutable(), $metaData );
		}
		catch ( Throwable $e )
		{
			$this->getEnv()->getErrorHandler()->captureException( $e );
